...
function useCurrentUser(name5) {
  if (!authUserMap.has(useFirebaseApp(name5))) {
    throw new Error(
      `[VueFire] useCurrentUser() called before the VueFireAuth module was added to the VueFire plugin. This will fail in production.`
    );
  }
  return authUserMap.get(useFirebaseApp(name5));
}
function useIsCurrentUserLoaded(name5) {
  const currentUser = useCurrentUser(name5);
...
function _getCurrentUserState(name5) {
  const firebaseApp = useFirebaseApp(name5);
...
function getCurrentUser(name5) {
  const userOrPromise = _getCurrentUserState(name5);
...
function useAppCheck(name5) {
  return AppCheckMap.get(useFirebaseApp(name5));
}
...
var Tree = class {
   ...
   */
  constructor(name5 = "", parent2 = null, node = { children: {}, childCount: 0 }) {
    this.name = name5;
    this.parent = parent2;
    this.node = node;
  }
...
function useDatabase(name5) {
  return getDatabase(useFirebaseApp(name5));
}
...
function useFirestore(name5) {
  return getFirestore(useFirebaseApp(name5));
}
...
function useFirebaseAuth(name5) {
  if (name5 != null) {
    console.warn(
...
function useFirebaseStorage(name5) {
  return getStorage(useFirebaseApp(name5));
}
